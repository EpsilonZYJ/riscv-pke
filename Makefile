...
USER_PERF_CPPS		:= user/app_test_performance.c user/user_lib.c
...
USER_PERF_OBJS  	:= $(addprefix $(OBJ_DIR)/, $(patsubst %.c,%.o,$(USER_PERF_CPPS)))
...
USER_PERF_TARGET	:= $(HOSTFS_ROOT)/bin/app_test_performance
...
$(USER_PERF_TARGET): $(OBJ_DIR) $(UTIL_LIB) $(USER_PERF_OBJS)
	@echo "linking" $@	...
	-@mkdir -p $(HOSTFS_ROOT)/bin
	@$(COMPILE) --entry=main $(USER_PERF_OBJS) $(UTIL_LIB) -o $@
	@echo "User app has been built into" \"$@\"
	@cp $@ $(OBJ_DIR)
...
all: $(KERNEL_TARGET) $(USER_PR_TARGET) $(USER_SC_TARGET) $(USER_MZSHELL_TARGET) $(USER_ZSHELL_TARGET) $(USER_SHELL_TARGET) $(USER_EXEC_TARGET) $(USER_RELA_TARGET) $(USER_COW_TARGET) $(USER_SEMA_TARGET) $(USER_WAIT_TARGET) $(USER_SUM_TARGET) $(USER_SING_TARGET) $(USER_PRINT_TARGET) $(USER_ERROR_TARGET) $(USER_PERF_TARGET) $(USER_MULTIMEM_TARGET0) $(USER_MULTIMEM_TARGET1) $(USER_MULTI_TARGET0) $(USER_MULTI_TARGET1) $(USER_E_TARGET) $(USER_M_TARGET) $(USER_T_TARGET) $(USER_C_TARGET) $(USER_O_TARGET)
.PHONY:all

run: $(KERNEL_TARGET) $(USER_PR_TARGET) $(USER_SC_TARGET) $(USER_MZSHELL_TARGET) $(USER_ZSHELL_TARGET) $(USER_SHELL_TARGET) $(USER_EXEC_TARGET) $(USER_RELA_TARGET) $(USER_COW_TARGET) $(USER_SEMA_TARGET) $(USER_WAIT_TARGET) $(USER_SUM_TARGET) $(USER_SING_TARGET) $(USER_PRINT_TARGET) $(USER_ERROR_TARGET) $(USER_PERF_TARGET) $(USER_MULTIMEM_TARGET0) $(USER_MULTIMEM_TARGET1) $(USER_MULTI_TARGET0) $(USER_MULTI_TARGET1) $(USER_E_TARGET) $(USER_M_TARGET) $(USER_T_TARGET) $(USER_C_TARGET) $(USER_O_TARGET)
	@echo "********************HUST PKE********************"
	@echo "********************APP SHELL*********************"

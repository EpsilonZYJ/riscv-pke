...
int spike_file_getdents(spike_file_t *f, char *dirp, uint32 count) {
    // 通过 FESVR 将 getdents 请求转发给宿主机
    return frontend_syscall(HTIFSYS_getdents, f->kfd, (uint64)dirp, count, 0, 0, 0, 0);
}
